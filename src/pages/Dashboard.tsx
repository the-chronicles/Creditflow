...
import { getMyLoans } from '../api/loan'; // Importing the getMyLoans function
...
const Dashboard = () => {
  ...
  // Fetching loans when the component mounts
  useEffect(() => {
  ...
  const totalOutstanding = loans.activeLoans.reduce((total, loan) => total + loan.remainingAmount, 0);
  const nextPaymentTotal = loans.activeLoans.reduce((total, loan) => total + loan.nextPaymentAmount, 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Across {loans.activeLoans.length} active loans</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Due on {new Date(loans.activeLoans[0]?.nextPayment).toLocaleDateString()}</p>
            </CardContent>
          </Card>
